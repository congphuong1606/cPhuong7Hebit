...
import com.discovertodo.phone.android.ebook.Content;
...
public class EbookFragment extends BaseFragment implements SensorEventListener {
    ...
    private int CODE_WRITE_SETTINGS_PERMISSION = 5;
    ...
    private void setTheme() {
        ...
        if (data.getIntValue("typelight") == 1) {
            type_light = data.getIntValue("typelight");
            typeLightWhite();
        }
        if (data.getIntValue("typelight") == 2) {
            type_light = data.getIntValue("typelight");
            typeLightSepia();
        }
    ...
    private void setupPopupMenu() {
        ...
        seekBarBrightness.setProgress(data.getIntValue("curBrightnessValue"));
        seekBarBrightness.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
            public void onStopTrackingTouch(SeekBar seekBar) {
                setBrightness(getActivity().getApplicationContext());
                data.setIntValue("curBrightnessValue", curBrightnessValue);
...
//		String color= "var paragraphs = document.getElementsByTagName('p');"+
    ...
    public void setBrightness(Context context) {
        ...
        if (ContextCompat.checkSelfPermission(getActivity(), Manifest.permission.WRITE_SETTINGS) == PackageManager.PERMISSION_GRANTED) {
            //code
            Settings.System.putInt(conresolver, Settings.System.SCREEN_BRIGHTNESS, curBrightnessValue);
            layoutParams.screenBrightness = curBrightnessValue / (float) 255;
            window.setAttributes(layoutParams);
        } else {
            boolean permission;
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                permission = Settings.System.canWrite(this.getContext());
            } else {
            ...
            if (permission) {
                ...
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
                    ...
                    getActivity().startActivityForResult(intent, CODE_WRITE_SETTINGS_PERMISSION);
                } else {
